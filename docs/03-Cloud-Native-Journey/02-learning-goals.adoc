...
[[LZ-3-2]]
==== LZ 3-2: Isolationskonzepte für Container und Container-Runtimes verstehen
...
Softwarearchitekt:innen kennen geeignete Sicherheitsmaßnahmen, um Risiken wie Container Escape und Privilegieneskalation beim Einsatz von Containern zu reduzieren, zum Beispiel:

* Policy- and Rule-based Runtime Security
* VM-isolated Container Runtimes
* Kernel-Proxy Runtimes
...
[[LG-3-2]]
==== LG 3-2: Understand Isolation Concepts for Containers and Container Runtimes
...
Software architects are familiar with suitable security measures to reduce risks such as container escape and privilege escalation when using containers, for example:
